...
// Hack to stop rusty_v8 being linked on docs.rs.
// rusty_v8 doesn't actually build v8 on docs.rs, which works since rustdoc never actually links the final code,
// but this build script depends on it when it's still compiled in docs.rs mode. and since this needs to be actually compiled,
// it'll fail with a linker error because none of the v8 functions are defined.
#[cfg(not(docsrs))]
mod inner {
